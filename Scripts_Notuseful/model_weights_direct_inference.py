...
import pandas as pd
import numpy as np
from weightflipping import inject_errors_into_state_dict, inject_errors_into_state_dict_all_lower_16, inject_errors_into_state_dict_remove_lower_16
...
state_dict = torch.load('/scratch/users/Maryam/Thesis_Tracking/L-DNQ/SmallerModels/essential_layers_model.pth')
# Checking the data types of weights
for key, tensor in state_dict.items():
    print(f"{key}: {tensor.dtype}")
# # Inject errors with a certain probability
flip_probability = 0.90  # For example, 10% of the weights will have bit flips
corrupted_state_dict = inject_errors_into_state_dict_remove_lower_16(state_dict, flip_probability)
...
df.to_excel('weight_directinefernce_error90_Lower16bitsremoved_with_metrics.xlsx', index=False)
...
plt.title('Confusion Matrix 90 Error Weights')
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')

# Save the confusion matrix plot
plt.savefig('Confusion_Matrix_90_Error_Weights.png')
plt.show()
